...
Elm.Action.make = function (_elm) {
   ...
   var iterate = F2(function (action,
   state) {
      return function () {
         switch (action.ctor)
         {case "MoveDown":
            ...
            return _U.replace([["cursor"
                               ...
                               state.cursor)]],
              state);
            case "UpdateFromBuffer":
            return A2($Model.insertAtCursor,
              action._0,
              state);}
         return state;
      }();
   ...
   var DeleteNextCharacter = {ctor: "DeleteNextCharacter"};
   var DeletePreviousCharacter = {ctor: "DeletePreviousCharacter"};
   var MergeBlocks = {ctor: "MergeBlocks"};
   var SplitBlock = {ctor: "SplitBlock"};
   var UpdateFromBuffer = function (a) {
      return {ctor: "UpdateFromBuffer"
             ,_0: a};
   };
   ...
   _elm.Action.values = {_op: _op
                        ,NoOp: NoOp
                        ,Reset: Reset
                        ,UpdateFromBuffer: UpdateFromBuffer
...
Elm.Core.make = function (_elm) {
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Native$DrawUtils = Elm.Native.DrawUtils.make(_elm),
   $Utils = Elm.Utils.make(_elm);
   var lineExtractor = F4(function (text,
   style,
   lineSize,
   linesAcc) {
      return function () {
         var _v0 = $Utils.unconsWord(text);
         switch (_v0.ctor)
         {case "Just":
            switch (_v0._0.ctor)
              {case "_Tuple2":
                 return function () {
                      var _v4 = $Utils.uncons(linesAcc);
                      switch (_v4.ctor)
                      {case "Just":
                         switch (_v4._0.ctor)
                           {case "_Tuple2":
                              return function () {
                                   var candidate = A2($Basics._op["++"],
                                   _v4._0._0,
                                   _v0._0._0);
                                   var candidateSize = A2($Native$DrawUtils.sizeOf,
                                   candidate,
                                   style);
                                   return _U.cmp(candidateSize,
                                   lineSize) < 1 ? A4(lineExtractor,
                                   _v0._0._1,
                                   style,
                                   lineSize,
                                   A2($List._op["::"],
                                   candidate,
                                   _v4._0._1)) : A4(lineExtractor,
                                   _v0._0._1,
                                   style,
                                   lineSize,
                                   A2($List._op["::"],
            ...
            return linesAcc;}
         _U.badCase($moduleName,
         "between lines 17 and 32");
      }();
   });
   var textToLines = F3(function (style,
   lineSize,
   text) {
      return $List.reverse(A4(lineExtractor,
      text,
      style,
      lineSize,
      _L.fromArray([""])));
...
Elm.LambdaEditor = Elm.LambdaEditor || {};
Elm.LambdaEditor.make = function (_elm) {
   "use strict";
   _elm.LambdaEditor = _elm.LambdaEditor || {};
   if (_elm.LambdaEditor.values)
   return _elm.LambdaEditor.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "LambdaEditor",
   ...
   var keyarrowToAction = function (_v0) {
      return function () {
         return function () {
            ...
            {case "_Tuple2": switch (_v2._0)
                 ...
                 {case -1:
                    ...
                    case 1: return $Action.MoveUp;}
                 break;}
            return $Action.NoOp;
         }();
      }();
   };
   var updates = $Signal.mergeMany(_L.fromArray([$Signal.subscribe($Views.updatesChannel)
                                                ,A2($Signal.map,
                                                keyarrowToAction,
                                                $Keyboard.arrows)]));
   var setup = _P.portIn("setup",
   function (v) {
      return v;
   });
   var stateWithSetup = function () {
      var _v5 = A2($Json$Decode.decodeValue,
      $Model.blocksDecoder,
      setup);
      switch (_v5.ctor)
      {case "Err":
   ...
   $Views.view,
   model);
   var documentUpdates = _P.portOut("documentUpdates",
   _P.outgoingSignal(function (v) {
      return v;
   }),
   A2($Signal.map,
   function ($) {
      return $Model.documentEncoder(function (_) {
         return _.document;
      }($));
   },
   model));
   _elm.LambdaEditor.values = {_op: _op
                              ,stateWithSetup: stateWithSetup
                              ,keyarrowToAction: keyarrowToAction
                              ,main: main
                              ,updates: updates
                              ,model: model};
   return _elm.LambdaEditor.values;
...
Elm.Model.make = function (_elm) {
   ...
   cursor) {
      return function () {
         var blockNo = cursor.block;
         var block = $Maybe.withDefault($List.head(blocks))(A2($Utils.lift,
         blockNo,
         blocks));
         var linesWithIndex = A2($List.indexedMap,
         F2(function (v0,v1) {
            return {ctor: "_Tuple2"
                   ,_0: v0
                   ,_1: v1};
               ...
               {case "_Tuple2":
                  switch (_v14._0)
                    ...
                    {case 0: return function () {
                            ...
                            $String.length,
                            A2($List.take,
                            $List.length(newBlock.lines) - 1,
                            newBlock.lines)));
                            var newX = startLastLine + posInLine;
                            return {_: {}
                                   ,block: newBlockNo
                                   ,x: newX};
                         }();}
                    break;}
               return function () {
                  var prevLine = $Maybe.withDefault($List.head(lines))(A2($Utils.lift,
                  lineNo - 1,
                  lines));
                  var newX = cursor.x - $String.length(prevLine);
                  return {_: {}
                         ,block: blockNo
                         ,x: newX};
               }();
   ...
   cursor) {
      return function () {
         return function () {
            ...
            lineNo,
            block.lines));
            return _U.cmp(lineNo,
            $List.length(block.lines) - 1) > -1 ? _U.cmp(blockNo,
            lastBlockNo) > -1 ? {_: {}
                                ,block: blockNo
                                ,x: blockLength(block)} : {_: {}
                                                          ,block: blockNo + 1
                                                          ,x: posInLine} : {_: {}
                                                                           ,block: blockNo
                                                                           ,x: cursor.x + $String.length(line)};
         }();
      }();
   });
   ...
   var Cursor = F2(function (a,b) {
      return {_: {},block: a,x: b};
   });
   ...
   var Bold = {ctor: "Bold"};
   var spanTypeDecoder = A2($Json$Decode.andThen,
   $Json$Decode.string,
   function (s) {
      return function () {
         switch (s)
         {case "bold":
            return $Json$Decode.succeed(Bold);
            case "italic":
            return $Json$Decode.succeed(Italic);}
         return $Json$Decode.fail("invalid span type");
      }();
   });
   var Span = F3(function (a,b,c) {
      return {_: {}
             ,end: b
             ,start: a
             ,type_: c};
   });
   var spanDecoder = A4($Json$Decode.object3,
   Span,
   A2($Json$Decode._op[":="],
   "start",
   $Json$Decode.$int),
   ...
   _elm.Model.values = {_op: _op
                       ...
                       ,cursorBlockLine: cursorBlockLine
                       ,cursorLineRec: cursorLineRec
...
Elm.Native.DrawUtils.make = function(elm) {
    ...
    function offscreen() {
        ...
        if (!os) {
            ...
            os.id = "le_offscreen";
            os.setAttribute('style', 'display: inline; position: absolute; top: 0; left: -999999px; width: 80000px; height: 100px;');
...
Elm.Utils.make = function (_elm) {
   ...
   $Maybe = Elm.Maybe.make(_elm),
   $String = Elm.String.make(_elm);
   var unconsWordRec = F2(function (acc,
   input) {
      return $String.isEmpty(acc) && $String.isEmpty(input) ? $Maybe.Nothing : function () {
         var _v0 = $String.uncons(input);
         switch (_v0.ctor)
         {case "Just":
            switch (_v0._0.ctor)
              {case "_Tuple2":
                 switch (_v0._0._0 + "")
                   {case " ":
                      return $Maybe.Just({ctor: "_Tuple2"
                                         ,_0: A2($Basics._op["++"],
                                         acc,
                                         $String.fromChar(_U.chr(" ")))
                                         ,_1: _v0._0._1});}
                   return A2(unconsWordRec,
                   A2($Basics._op["++"],
                   acc,
                   $String.fromChar(_v0._0._0)),
                   _v0._0._1);}
              break;
            case "Nothing":
            return $Maybe.Just({ctor: "_Tuple2"
   ...
   _elm.Utils.values = {_op: _op
                       ...
                       ,tail: tail
                       ,uncons: uncons
                       ,unconsWord: unconsWord
                       ,unconsWordRec: unconsWordRec};
   return _elm.Utils.values;
};
Elm.Views = Elm.Views || {};
Elm.Views.make = function (_elm) {
   ...
   $Utils = Elm.Utils.make(_elm);
   var linePositions = function (line) {
      return function () {
         var length = $String.length(line);
         var substrings = A2($List.map,
         function (i) {
            return A2($String.left,
            i,
            line);
         },
         _L.range(0,length));
         return A2($List.map,
         function (s) {
            return A2($Native$DrawUtils.sizeOf,
            s,
            "");
         },
         substrings);
   ...
   var lineViewHeight = function (lineView) {
      return $Graphics$Element.heightOf(lineView.element);
   };
   var blockViewHeight = function (blockView) {
      return $List.sum(A2($List.map,
      lineViewHeight,
      blockView.lineViews));
   };
   var renderBlock = function (_v0) {
      return function () {
   ...
   var offsetLines = F2(function (offset,
   lines) {
      return function () {
         switch (lines.ctor)
         {case "::":
   ...
   var rawText = $Text.defaultStyle;
   var spanStyle = function (style) {
      return function () {
         switch (style.ctor)
         {case "Bold":
            return _U.replace([["bold"
                               ,true]],
              rawText);
            case "Italic":
            return _U.replace([["italic"
                               ,true]],
              rawText);}
         _U.badCase($moduleName,
   ...
   text) {
      return function () {
         var min = A2($List.foldl,
         F2(function (span,maybeMin) {
            return function () {
               switch (maybeMin.ctor)
               {case "Just":
                  return _U.cmp(span.start,
                    maybeMin._0.start) < 0 ? $Maybe.Just(span) : $Maybe.Just(maybeMin._0);
                  case "Nothing":
   ...
   var BlockView = function (a) {
      return {_: {},lineViews: a};
   };
   var DocumentView = function (a) {
      return {_: {},blockViews: a};
   };
   var cursorWidth = 2;
   var cursorElement = function (height) {
      return A2($Graphics$Collage.collage,
      cursorWidth,
      height)(function (f) {
         return _L.fromArray([f]);
      }($Graphics$Collage.filled($Color.black)(A2($Graphics$Collage.rect,
      $Basics.toFloat(cursorWidth),
      ...
      return function () {
         return function () {
            return function () {
               ...
               lineNo,
               blockView.lineViews);
               var lineView = $Maybe.withDefault($List.head(blockView.lineViews))(A2($Utils.lift,
               lineNo,
               blockView.lineViews));
               var cursorElt = cursorElement(lineViewHeight(lineView));
               var pixelX = $Maybe.withDefault(0)(A2($Utils.lift,
               posInLine,
               linePositions(lineView.line)));
               var pixelY = $List.sum(A2($List.map,
               blockViewHeight,
               blocksBefore)) + blockNo * interblock + $List.sum(A2($List.map,
               lineViewHeight,
               linesBefore));
               ...
               $Graphics$Element.Absolute(pixelX),
               $Graphics$Element.Absolute(pixelY)),
               cursorElt);
            }();
         }();
      }();
   ...
   var view = function (state) {
      return function () {
         var documentView = buildDocument(state.document);
         var cursorView = A3(buildCursor,
         state.document,
         documentView,
         state.cursor);
         var complete = $Graphics$Element.layers(_L.fromArray([renderDocument(documentView)
                                                              ,cursorView]));
         return A2($Html.div,
   ...
   _elm.Views.values = {_op: _op
                       ...
                       ,lineViewHeight: lineViewHeight
                       ,linePositions: linePositions
                       ,blockViewHeight: blockViewHeight
